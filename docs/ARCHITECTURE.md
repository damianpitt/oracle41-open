...
`ProtocolPortfolioService` prices the underlying assets in the newest stored snapshot for every protocol on each wallet and chain. It uses the same cached pricing boundary as wallet balances. Supplied assets and collateral add to asset value; debt adds to liabilities and subtracts from net value. Missing prices and partial snapshots prevent a complete total but do not hide known values.
