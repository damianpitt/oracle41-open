...
The Portfolio view reads snapshot history directly from SQLite. Choosing an exact block does not contact a provider. Wallet overview providers still return current balances, so exact protocol-block mode does not claim a complete historical portfolio total. It keeps the mixed-time known estimate visible with the requested block. The separate protocol refresh action calls `ProtocolPositionService` with forced refresh, recollects Aave state at that block, replaces the stored result, and then rebuilds the portfolio. Protocol-position CSV and JSON exports are created from the same valued rows shown by the view.
