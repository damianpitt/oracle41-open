...
`ProtocolPositionService` coordinates production lending collection through the shared transaction-provider interface. The Aave collector discovers reserves and reads balances, configuration, account health, and oracle units. The Compound collector handles each Comet market separately and reads base balances, collateral configuration, collateral balances, and native safety checks. Every successful read in one snapshot must use the same provider and exact block. Discovery is required; later optional failures produce a partial result while keeping valid evidence.
...
The Aave V3 adapter recognizes official Pool deployments on Ethereum, Optimism, Polygon, Base, and Arbitrum. It converts the collected reserve calls into supplied, collateral, and debt positions. A collateral position is still one supplied balance, so it is not also emitted as a second supplied position. Stable and variable debt use the same underlying token and are combined into one debt amount. Account totals, loan-to-value, liquidation threshold, and health factor stay in Aave's original integer units. The adapter does not calculate prices or give financial advice. Storage and portfolio valuation remain separate services so adapter output stays deterministic.

The Compound V3 adapter recognizes configured Comet proxies on the same five chains. Each Comet market has one base asset and a separate collateral list, so every market receives its own adapter identity, checkpoint, and stored snapshot. Base supply, base debt, and collateral become positions. Compound's collateralized and liquidatable checks remain native Boolean evidence. Oracle41 does not create unsupported health-factor or account-level collateral values.

`ProtocolPortfolioService` prices the underlying assets in the newest stored snapshot for every protocol on each wallet and chain, or in snapshots stored at one requested block. It uses the same cached pricing boundary as wallet balances. Supplied assets and collateral add to asset value; debt adds to liabilities and subtracts from net value. Missing prices, missing exact snapshots, partial snapshots, stale observations, and future timestamps prevent a complete total but do not hide known values.

The service creates one immutable risk report for each protocol snapshot. The report keeps raw collateral, debt, available borrow, health factor, loan-to-value, liquidation threshold, warnings, adapter identity, source reference, provider, observation time, storage time, and freshness threshold. A supplied clock makes age classification deterministic in tests. Freshness describes the age of the provider observation, not the distance from the current chain head.

Aave aTokens and debt tokens mirror economic positions rather than adding separate value. A Compound Comet token can likewise mirror positive base supply. The service removes these matching wallet balances only when positive protocol evidence proves that their underlying economic position is included. The stored amount belongs to its explicit snapshot block, while its USD quote is the latest available price. Historical price-at-block valuation remains separate future work.

The Portfolio view reads snapshot history directly from SQLite. Choosing an exact block does not contact a provider. Wallet overview providers still return current balances, so exact protocol-block mode does not claim a complete historical portfolio total. It keeps the mixed-time known estimate visible with the requested block. The separate protocol refresh action calls `ProtocolPositionService` with forced refresh, recollects supported Aave and Compound state at that block, replaces stored results, and then rebuilds the portfolio. Protocol-position and protocol-risk CSV and JSON exports are created from the same reports shown by the view.
