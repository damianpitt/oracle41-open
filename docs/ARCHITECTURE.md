...
Aave aTokens and debt tokens mirror economic positions rather than adding separate value. The service reads their addresses only from positive reserve evidence and removes matching wallet balances before adding the underlying position. This avoids counting both a receipt token and its supplied asset. The stored amount belongs to its explicit snapshot block, while its USD quote is the latest available price. Historical price-at-block valuation remains separate future work.

The Portfolio view reads snapshot history directly from SQLite. Choosing an exact block does not contact a provider. Wallet overview providers still return current balances, so exact protocol-block mode does not claim a complete historical portfolio total. It keeps the mixed-time known estimate visible with the requested block. The separate protocol refresh action calls `ProtocolPositionService` with forced refresh, recollects Aave state at that block, replaces the stored result, and then rebuilds the portfolio. Protocol-position and protocol-risk CSV and JSON exports are created from the same reports shown by the view.
