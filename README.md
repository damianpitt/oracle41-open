...
Version `0.4.0a10` is an alpha release. Alchemy, Ankr, Moralis, and GoldRush can be enabled and ordered in Settings. All four adapters pass the same recorded-fixture checks for normalized wallet operations. Oracle41 can collect, resume, save, price, and aggregate block-specific Aave V3 supplied assets, collateral, debt, and account health data on every supported chain. The Portfolio view reports protocol assets, liabilities, net value, provenance, missing prices, and receipt-token exclusions.
...
- Aave V3 positions use current available prices for stored snapshot amounts. Historical price-at-block valuation and dedicated protocol-position export templates are not included yet.
